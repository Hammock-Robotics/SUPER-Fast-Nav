...
# V3 + change receiving heading commands as well
import rospy
...
class WaypointNavigator:
    ...
    def advance_to_next_waypoint(self):
        ...
        goal = self.convert_to_local_goal(wp)
    ...
    def convert_to_local_goal(self, waypoint, initial_orientation=False):
        # convert GPS to local XY offset
        ...
        local_x = self.origin_local_x + dx
        local_y = self.origin_local_y + dy

        yaw_rad = self.wp_headings[self.current_goal_index]
        quat = transformations.quaternion_from_euler(0, 0, yaw_rad)

        pose = PoseStamped()
        pose.header.frame_id = "world"              # use world frame (not "map")
        pose.pose.position.x = local_x
        pose.pose.position.y = local_y
        pose.pose.position.z = waypoint.z_alt       # use absolute altitude from mission planner
    ...
    def update_goal_if_needed(self):
        ...
        initial = (g['s12'] >= 1.0)
        goal = self.convert_to_local_goal(wp, initial_orientation=initial)
        goal.header.stamp = rospy.Time.now()
        self.goal_pub.publish(goal)
        self.last_update_position = (self.current_local_x, self.current_local_y)
