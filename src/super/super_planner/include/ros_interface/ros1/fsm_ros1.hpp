...
namespace fsm {
    class FsmRos1 : public Fsm {
        ...
        ros::Publisher state_pub;   // ← new one i added
        ros::Subscriber mode_sub_;  // ← new one i added
    ...
    public:
        ...
        bool getPoseFromTraj(super_utils::Pose &pose) {
            ...
            if (traj_finish_) {
                ...
                if (closeToGoal(0.4)) {  //ori 0.1 i changed this
        ...
        void init(const ros::NodeHandle &nh, const std::string &cfg_path) {
            ...
            state_pub = nh_.advertise<std_msgs::String>("/super_fsm_state", 10); //new one i added
            mode_sub_ = nh_.subscribe<mavros_msgs::State>("/mavros/state", 10, &FsmRos1::mavrosStateCallback, this); //new one i added
        ...
        void replanTimerCallback(const ros::TimerEvent &event) {
            callReplanOnce();
            // i added this, publish new FSM state
            std_msgs::String s;
            s.data = MACHINE_STATE_STR[int(machine_state_)];
            state_pub.publish(s);
        ...
        // I ADDED THIS !!
        void mavrosStateCallback(const mavros_msgs::StateConstPtr &m) {
            static std::string last = "";
            // detect GUIDED→LOITER transition
            // or just GUIDED-not GUIDED
            if (last == "GUIDED" && m->mode == "LOITER") {
                ChangeState("mavrosMode", PAUSING); // change mavrosMode to PAUSING
                std_msgs::String st;
                st.data = MACHINE_STATE_STR[int(machine_state_)];
